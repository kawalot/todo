...
class ListAPITest(TestCase):
    ...
    def test_get_returns_items_for_correct_list(self):
        ...
        Item.objects.create(lst=other_list, text='item 1')
        our_list = List.objects.create()
        item1 = Item.objects.create(lst=our_list, text='item 1')
        item2 = Item.objects.create(lst=our_list, text='item 2')
        response = self.client.get(self.base_url.format(our_list.id))
        self.assertEqual(
            json.loads(response.content.decode('utf8')),
            {'id': our_list.id, 'items': [
                {'id': item1.id, 'lst': our_list.id, 'text': item1.text},
                {'id': item2.id, 'lst': our_list.id, 'text': item2.text},
            ]}
        )
